...
# if !defined(OS2) && !defined(PCR) && !defined(AMIGA) \
     ...
     && !defined(__CC_ARM) && !defined(_XBOX)
...
#if defined(_XBOX)  
void* xenon_get_mem (size_t size, size_t page_size)
{
	if (size)
	{
		void* p = VirtualAlloc(NULL, size, MEM_COMMIT | MEM_NOZERO | MEM_TOP_DOWN, PAGE_READWRITE);
		return p;				
	}
	else
	{
...
# if !defined(USE_WINALLOC) && !defined(_XBOX)
...
#if !defined(MSWIN32) && !defined(MSWINCE) && !defined(_XBOX) && !defined(_XBOX_ONE) && !defined(NN_PLATFORM_CTR)
# include <unistd.h>
#ifdef SN_TARGET_PS3
