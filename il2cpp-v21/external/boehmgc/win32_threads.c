...
//#define XBOX_ENABLE_XBDM
#if defined(XBOX_ENABLE_XBDM)
#include <xbdm.h>
#endif
...
# if defined(_XBOX)
	...
	{
		ptr_t smin = ((ptr_t)(((DWORD)(lower_bound) - 1) & 0xFFFFF000)); // next lowest multiple of 4K
		return smin;
	}

	/*
	// Accurate method to get stack limit:
	//static ptr_t GC_get_stack_min(DWORD tid)
	//{

	//	DM_THREADINFOEX tinfo;
	//	HRESULT hr;
	//	memset(&tinfo, 0, sizeof(DM_THREADINFOEX));
	//	tinfo.Size = sizeof(DM_THREADINFOEX);
	//	hr = DmGetThreadInfoEx(tid, &tinfo);
	//	return tinfo.StackLimit;
	//}
	*/
...
#	elif defined(_XBOX)
	  for (_reg = 14; _reg <= 31; ++_reg)
		GC_push_one((word)_r14_r31[_reg-14]);
	  sp = (ptr_t)_r1;
#	elif defined(ALPHA)
